...
export default function FeedbackPage() {
  ...
  const steps = [
    {
      eyebrow: 'A little context', title: 'Let’s start with your child’s journey.', cardTitle: 'Begin with the whole story', cardText: 'A little context helps us meet your child where they are today and understand the season they are moving through.', cardNote: 'Context gives every answer a clearer meaning.',
      content: <>
        ...
        <label className="feedback-field"><span>Child’s name</span><input name="childName" value={form.childName} onChange={update} required placeholder="Enter your child’s name" /></label>
        <fieldset className="feedback-fieldset"><legend>How long has your child been receiving mentoring or coaching from us?</legend><div className="feedback-radio-grid">{['Less than 1 month', '1–3 months', '3–6 months', 'More than 6 months'].map((option) => <label className="feedback-radio" key={option}><input type="radio" name="duration" value={option} checked={form.duration === option} onChange={update} required /><span>{option}</span></label>)}</div></fieldset>
      </>
    },
    {
      eyebrow: 'Notice the shifts', title: 'What positive changes have you noticed?', cardTitle: 'Name the wins', cardText: 'Progress is often found in the small, everyday moments. Your observations help us celebrate the changes your child may not yet see.', cardNote: 'Noticing progress gives it room to grow.',
    ...
    },
    {
      ...
      content: <><div className="feedback-two-col"><label className="feedback-field"><span>How satisfied are you?</span><select name="satisfaction" value={form.satisfaction} onChange={update} required><option value="">Choose one</option>{['Very satisfied', 'Satisfied', 'Fairly satisfied', 'Not satisfied'].map((option) => <option key={option}>{option}</option>)}</select></label><label className="feedback-field"><span>How would you rate the coach’s relationship with your child?</span><select name="relationship" value={form.relationship} onChange={update} required><option value="">Choose one</option>{['Excellent', 'Very good', 'Good', 'Needs improvement'].map((option) => <option key={option}>{option}</option>)}</select></label></div><label className="feedback-field"><span>What does your child say about the sessions?</span><textarea name="childSays" value={form.childSays} onChange={update} rows="3" /></label><label className="feedback-field"><span>Anything the coach should know about your child’s current development or behaviour?</span><textarea name="development" value={form.development} onChange={update} rows="3" /></label><label className="feedback-field"><span>What can we do differently to make this more valuable?</span><textarea name="valuable" value={form.valuable} onChange={update} rows="3" /></label><fieldset className="feedback-fieldset"><legend>Would you recommend Paz Thriving Tribe to another parent?</legend><div className="feedback-radio-grid compact">{['Definitely', 'Probably', 'Not sure', 'No'].map((option) => <label className="feedback-radio" key={option}><input type="radio" name="recommend" value={option} checked={form.recommend === option} onChange={update} required /><span>{option}</span></label>)}</div></fieldset><label className="feedback-field"><span>Testimonial (optional)</span><textarea name="testimonial" value={form.testimonial} onChange={update} rows="3" placeholder="In a few words, how has Paz Thriving Tribe impacted your child?" /></label></>
    }
  ];
  ...
  if (submitted) return <main className="feedback-page feedback-success-page"><div className="feedback-firework feedback-firework-one" /><div className="feedback-firework feedback-firework-two" /><section className="feedback-success-backdrop"><div className="feedback-complete" aria-labelledby="feedback-success-title"><div className="feedback-thankyou-art" aria-hidden="true"><span className="thankyou-spark spark-a">✦</span><span className="thankyou-spark spark-b">✦</span><span className="thankyou-sun" /><span className="thankyou-person"><i /><b /></span><span className="thankyou-heart">♥</span></div><div className="feedback-complete-mark">✓</div><p className="feedback-kicker">Feedback received</p><h1 id="feedback-success-title">Thank you for helping children thrive.</h1><p>Your thoughtful reflections help us understand what is working and where your child needs more care, attention, and encouragement.</p><Link className="feedback-primary-btn" to="/">Return to Paz Thriving Tribe <span>→</span></Link></div></section></main>;

  return (
    <main className={`feedback-page ${introDone ? 'intro-finished' : ''}`}>
      {!introDone && <section className={`feedback-intro ${introReady ? 'ready' : ''}`} aria-label="Why your feedback matters"><div className="feedback-intro-art"><div className="feedback-sun" /><div className="feedback-orbit orbit-one" /><div className="feedback-orbit orbit-two" /><div className="feedback-note note-one">listen</div><div className="feedback-note note-two">notice</div><div className="feedback-spark spark-one">✦</div><div className="feedback-spark spark-two">✦</div><div className="feedback-figure"><span className="figure-head" /><span className="figure-body" /><span className="figure-arm" /></div></div><div className="feedback-intro-copy"><span className="feedback-kicker">Paz Thriving Tribe · Parent voice</span><h1>Every child’s growth has a story.</h1><p>Your perspective helps us see the progress that happens between sessions, celebrate what is changing, and shape better support for the road ahead.</p><div className="feedback-intro-actions"><div className="feedback-loading-line"><span /><small>{introReady ? 'Your reflection space is ready' : 'Preparing your reflection space'}</small></div><button type="button" className="feedback-primary-btn feedback-start-btn" onClick={startSurvey} disabled={!introReady}>Take the survey <span>→</span></button></div></div><div className={`feedback-arrow ${introReady ? 'visible' : ''}`}><span>Begin here</span><strong>↘</strong></div></section>}
