...
export default function FeedbackPage() {
  ...
  const steps = [
    {
      eyebrow: 'A little context', title: 'Let’s start with your Pre-teen/Teens’ journey.', cardTitle: 'Begin with the whole story', cardText: 'A little context helps us understand where your Pre-teen/Teens are today and the season they are moving through.', cardNote: 'Context gives every answer a clearer meaning.',
      content: <>
        ...
        <label className="feedback-field"><span>Gender</span><CustomDropdown value={form.gender} onChange={(value) => update({ target: { name: 'gender', value } })} ariaLabel="Gender" placeholder="Select gender" options={[{ value: '', label: 'Select gender' }, { value: 'male', label: 'Male' }, { value: 'female', label: 'Female' }]} /></label>
    ...
    {
      eyebrow: 'Notice the shifts', title: 'What positive changes have you noticed?', cardTitle: 'Name the wins', cardText: 'Progress is often found in the small, everyday moments. Your observations help us celebrate the changes your Pre-teen/Teens may not yet see.', cardNote: 'Noticing progress gives it room to grow.',
    ...
    },
    {
      ...
      content: <><div className="feedback-two-col"><label className="feedback-field"><span>How satisfied are you?</span><CustomDropdown value={form.satisfaction} onChange={(value) => update({ target: { name: 'satisfaction', value } })} ariaLabel="Satisfaction" placeholder="Choose one" options={['Very satisfied', 'Satisfied', 'Fairly satisfied', 'Not satisfied'].map((option) => ({ value: option, label: option }))} /></label><label className="feedback-field"><span>How would you rate the coach’s relationship with your Pre-teen/Teens?</span><CustomDropdown value={form.relationship} onChange={(value) => update({ target: { name: 'relationship', value } })} ariaLabel="Coach relationship" placeholder="Choose one" options={['Excellent', 'Very good', 'Good', 'Needs improvement'].map((option) => ({ value: option, label: option }))} /></label></div><label className="feedback-field"><span>What do your Pre-teen/Teens say about the sessions?</span><textarea name="childSays" value={form.childSays} onChange={update} rows="3" /></label><label className="feedback-field"><span>Anything the coach should know about your Pre-teen/Teens’ current development or behaviour?</span><textarea name="development" value={form.development} onChange={update} rows="3" /></label><label className="feedback-field"><span>What can we do differently to make this more valuable?</span><textarea name="valuable" value={form.valuable} onChange={update} rows="3" /></label><fieldset className="feedback-fieldset"><legend>Would you recommend Paz Thriving Tribe to another parent?</legend><div className="feedback-radio-grid compact">{['Definitely', 'Probably', 'Not sure', 'No'].map((option) => <label className="feedback-radio" key={option}><input type="radio" name="recommend" value={option} checked={form.recommend === option} onChange={update} required /><span>{option}</span></label>)}</div></fieldset><label className="feedback-field"><span>Testimonial (optional)</span><textarea name="testimonial" value={form.testimonial} onChange={update} rows="3" placeholder="In a few words, how has Paz Thriving Tribe impacted your Pre-teen/Teens?" /></label></>
    }
  ];
