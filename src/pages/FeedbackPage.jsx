...
export default function FeedbackPage() {
  ...
  const steps = [
    ...
    },
    {
      ...
      content: <><div className="feedback-two-col"><label className="feedback-field"><span>How satisfied are you?</span><select name="satisfaction" value={form.satisfaction} onChange={update} required><option value="">Choose one</option>{['Very satisfied', 'Satisfied', 'Fairly satisfied', 'Not satisfied'].map((option) => <option key={option}>{option}</option>)}</select></label><label className="feedback-field"><span>How would you rate the coach’s relationship with your Pre-teen/Teens?</span><select name="relationship" value={form.relationship} onChange={update} required><option value="">Choose one</option>{['Excellent', 'Very good', 'Good', 'Needs improvement'].map((option) => <option key={option}>{option}</option>)}</select></label></div><label className="feedback-field"><span>What do your Pre-teen/Teens say about the sessions?</span><textarea name="childSays" value={form.childSays} onChange={update} rows="3" /></label><label className="feedback-field"><span>Anything the coach should know about your Pre-teen/Teens’ current development or behaviour?</span><textarea name="development" value={form.development} onChange={update} rows="3" /></label><label className="feedback-field"><span>What can we do differently to make this more valuable?</span><textarea name="valuable" value={form.valuable} onChange={update} rows="3" /></label><fieldset className="feedback-fieldset"><legend>Would you recommend Paz Thriving Tribe to another parent?</legend><div className="feedback-radio-grid compact">{['Definitely', 'Probably', 'Not sure', 'No'].map((option) => <label className="feedback-radio" key={option}><input type="radio" name="recommend" value={option} checked={form.recommend === option} onChange={update} required /><span>{option}</span></label>)}</div></fieldset><label className="feedback-field"><span>Testimonial (optional)</span><textarea name="testimonial" value={form.testimonial} onChange={update} rows="3" placeholder="In a few words, how has Paz Thriving Tribe impacted your Pre-teen/Teens?" /></label></>
    }
  ];
